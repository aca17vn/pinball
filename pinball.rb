...
def update(racquet,screen,testmode)
   ...
   new_x = $x
   new_y = $y
   new_x = (next_x+$dx).floor
   new_y = (next_y+$dy).floor-1
   if (new_x <= racquet+3) && (new_x >= racquet - 1) && (new_y == 13)

      $dy = -$dy

   elsif (screen[new_y][new_x] == SC_H)

      $dy = -$dy

   elsif (screen[new_y][new_x] == SC_V)
...
def displayDyn(screen,racquet)
  ...
  if $y.between?(0,SCREEN_Y) && $x.between?(0,SCREEN_X) 
  ...
  if racquet.between?(1, SCREEN_X)

		# erases the old racquet position

    (2..SCREEN_X - 1).each do |column| 

      print "\e[#{SCREEN_Y};#{column}H-"

    end

		# displays the new position

    (1...SCREEN_X).each do |column|

      if column.between?(racquet + 1, (racquet + 4)) 
